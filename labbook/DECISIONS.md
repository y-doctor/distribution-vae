...
## [2026-04-19] hinge_multiplier=2.0 as the default on 2kg/singles
**Context**: Three consecutive runs under hinge_multiplier=1.0 (binary hinge, rescale-short, rescale-long) all hit P(r ≥ 0.9) ≈ 0.12 on held-out, and I had hypothesized this was a biology ceiling from reward-degenerate pairs in the 105-pert vocab. The rescale-long entry (2026-04-19_2142) even recommended entropy-floor regularization as the primary remedy for the apparent ceiling.
**Decision**: Default to `hinge_multiplier=2.0` + `hinge_rescale=true` on the 2kg/singles MLP line. The 2× variant plateau-stopped at ep 387 with held-out ens=10 top-1 **0.174** (vs 0.111 under 1× long, +57% rel) and **P(r ≥ 0.9) 0.188** (vs 0.119, +58% rel), breaking the "ceiling." Broad-class performance (P(r ≥ 0.5)) preserved at ~0.53.
**Alternatives considered**:
- **Keep 1× hinge + add entropy floor**: was the leading recommendation before the 2× A/B. Likely still worth doing, but 2× alone delivered bigger lift than entropy regularization would have (P(r ≥ 0.9) gains are unachievable without removing the plateau trap).
- **3× hinge**: untested. Moves further in the same direction but with rising dead-group risk for weak-signal perts (BCL2L11, MAP4K3 have ≤5 neighbors above r=0.5). Flagged as next experiment.
- **Aggressive entropy regularizer only**: would widen above-threshold sampling but leaves the plateau trap intact — the policy could still converge to modest-reward plateau members with more exploration, just more slowly.
**Rationale**: The "biology ceiling" narrative at P(r ≥ 0.9) was a misdiagnosis. The real ceiling was the policy settling into a modest-reward plateau (41+ sorta-related perts with r ∈ [0.21, 0.50] for isolated perts like BCL2L11) as an easy local optimum. Doubling the hinge zeros that plateau out — only genuinely-related perts get reward — forcing the policy into the fine-discrimination regime. Training-side: train top-1 at ep 150 under 2× is 0.565, already above the 1×-long run's final 0.555 at ep 405 — 2.5× sample efficiency for the same training signal. No dead-group regression observed during training. See `labbook/entries/2026-04-19_2226_hinge2x_broke_the_ceiling.md`.
